...
class NMTModel(object):
    ...
    def __init__(self,
        ...
        assert attention_type.lower() in ('bahdanau', 'luong'), 'attention_type必须是bahdanau或者luong，而不是{}'.format(
            attention_type.lower())
        assert beam_width < target_vocab_size, 'beam_width {}应该小于target_vocab_size {}'.format(beam_width,
                                                                                              target_vocab_size)  # 这个需要验证参数对不对
    ...
    def init_optimizer(self):
        ...
        self.updates1 = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss=self.loss,
                                                                                     global_step=self.global_step)
